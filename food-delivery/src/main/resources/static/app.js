...
// We use a relative path ("/api") instead of "http://localhost:8080/api"
// This ensures it works on both your Localhost AND the Cloud automatically.
const API_BASE = "/api";
...
async function loadMenu() {
    ...
    try {
        ...
        // Auto-seed if database is empty (runs once on deployment)
        if (data.length === 0) {
            await seedDatabase();
            return;
        }
...
function renderMenu(items) {
    ...
    items.forEach((item, index) => {
        ...
        card.innerHTML = `
            <img src="${item.imageUrl}" alt="${item.name}" onerror="this.src='https://via.placeholder.com/300?text=No+Image'">
            <div class="card-body">
